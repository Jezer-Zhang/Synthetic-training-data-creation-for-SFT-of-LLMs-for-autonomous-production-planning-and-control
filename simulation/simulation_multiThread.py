...
class Simulator:
    def __init__(
        ...
        self.ro_steps = []
        self.docking = True
        self.robotino_docked_with_island_ii = False
        self.robotino_docked_with_island_iii = False

        self.robo_dock = False # islands check if robotino has already docked.
        ...
        self.threads = []
        # self.event_lock = threading.Lock()  # 添加一个锁来处理事件
    ...
    def get_order_steps(self):
        ...
        for order_str in orders:
            try:
                ...
                logging.error(f"Error decoding the JSON string: {order_str}")
        return None
    ...
    def process_new_event(self, agent):
        try:
            ...
            # # 检查是否是 Robotino 的特定事件
            # if new_event and any("[Robotino]" in event for event in new_event) and any("PG73 recieves request" in event for event in new_event):
            #     robotino_agent = self.get_agent("Robotino")
            #     robotino_thread = threading.Thread(
            #         target=self.run_agent, args=(robotino_agent, {"Details": self.robo_details})
            #     )
            #     self.threads.append(robotino_thread)
            #     robotino_thread.start()

            if new_event and self.docking and not self.robotino_docked_with_island_ii and any("Robotino has docked with Island II." in event for event in new_event):                
                self.robotino_docked_with_island_ii = True
                island_ii = self.get_agent("Island II")
                island_ii_step = next(step for step in self.steps if step["Agent"] == "Island II")
                island_ii_thread = threading.Thread(
                    target=self.run_agent, args=(island_ii, island_ii_step)
                )
                self.threads.append(island_ii_thread)
                island_ii_thread.start()

            if new_event and self.docking and not self.robotino_docked_with_island_iii and any("Robotino has docked with Island III." in event for event in new_event):
                self.robotino_docked_with_island_iii = True
                island_iii = self.get_agent("Island III")
                island_iii_step = next(step for step in self.steps if step["Agent"] == "Island III")
                island_iii_thread = threading.Thread(
                    target=self.run_agent, args=(island_iii, island_iii_step)
                )
                self.threads.append(island_iii_thread)
                island_iii_thread.start()
            ...
            if new_event:  # 最后一步返回 null， sop流程结束
                # with self.event_lock:
                    ...
                    if event_log is not None:
                        ...
                        if command_name:
                            if "robot_request" in command_name and "robot_request('Island I', 'in')" in command_name:
                                robotino_agent = self.get_agent("Robotino")
                                robotino_thread = threading.Thread(
                                    target=self.run_agent, args=(robotino_agent, self.ro_steps.pop(0))
                                )
                                self.threads.append(robotino_thread)
                                robotino_thread.start()

                            elif "robot_request" in command_name and "robot_request('Island II', 'in')" in command_name:
                                robotino_agent = self.get_agent("Robotino")
                                robotino_thread = threading.Thread(
                                    target=self.run_agent, args=(robotino_agent, self.ro_steps.pop(0))
                                )
                            ...
                            elif "robot_request" in command_name and "robot_request('Island III', 'in')" in command_name:
                                robotino_agent = self.get_agent("Robotino")
                                robotino_thread = threading.Thread(
                                    target=self.run_agent, args=(robotino_agent, self.ro_steps.pop(0))
                                )
    ...
    def simulate_process(self):
        ...
        production_steps = self.get_order_steps()

        for step in production_steps:
            self.steps.append(step)
            agent_name = step["Agent"]
            self.agent_seq.append(agent_name)
            if agent_name == "Robotino":
                self.ro_steps.append(step)
        ...
        #             target=self.run_agent, args=(agent, step)
        #         )
        #         self.threads.append(agent_thread)
        #         agent_thread.start()
        
        self.run_agent(self.island_1, self.steps[0] )

        for thread in self.threads:
            thread.join()

    def run_agent(self, agent, step):


        if agent.agent_name == "Island I":
            event = f"[{agent.agent_name}] [{datetime.now().strftime('%H:%M:%S')}] BG56 detects a workpiece at the infeed of conveyor C1."
            self.event_log.add_event(event, agent)

        elif agent.agent_name == "Robotino":
            details = step["Details"]    
            event = f"[{agent.agent_name}] [{datetime.now().strftime('%H:%M:%S')}] PG73 recieves request from {details[0]["From"]} to transport workpieces."
            agent.ro_destination = details[1]["To"]
            self.event_log.add_event(event, agent)

        elif agent.agent_name == "Island II":
            event = f"[{agent.agent_name}] [{datetime.now().strftime('%H:%M:%S')}] Robotino has docked with Island II."
            self.event_log.add_event(event, agent)

        elif agent.agent_name == "Island III":
            event = f"[{agent.agent_name}] [{datetime.now().strftime('%H:%M:%S')}] Robotino has docked with Island III."
            self.event_log.add_event(event, agent)

        while self.running:
            self.process_new_event(agent)
            time.sleep(1)  # Simulate some delay for processing
...
class OperationAgent(Agent):
    ...
    def robot_request(self, destination, mode):
        if destination in ["Island I", "Island II"] and mode == "in":
            # robotino_event = f"[Robotino] [{datetime.now().strftime('%H:%M:%S')}] PG73 recieves request from {self.agent_name} to transport workpieces."
            # self.event_log.add_event(robotino_event, self.event_log.simulator.robotino, NOTIFY = False)
            pass         
            # if destination == "Island I": 
            #     self.event_log.simulator.robo_details = [{"From": "Island I"},{"To": "Island II"}]
            # elif destination == "Island II": 
            #     self.event_log.simulator.robo_details = [{"From": "Island II"},{"To": "Island III"}]

    def no_action(self): 
        self.event_log.simulator.running = False
        return None
    ...
    def alert_to_supervisor(self, reason):
        # supervisor module?
        return None
    ...
    def navigate_to(self, destination, mode):
        time.sleep(12)
        if mode == "in":
            self.event_log.simulator.docking = False
        elif mode == "out": 
            self.event_log.simulator.docking = True 
        
        self.event_log.simulator.robo_dock = True
...
if __name__ == "__main__":
    ...
    simulator = Simulator(
        island_1, island_2, island_3, robotino, event_log, max_events=300, max_time=300
    )
    ...
    eventLog_path = "./eventLog/event_log_multi.json"
